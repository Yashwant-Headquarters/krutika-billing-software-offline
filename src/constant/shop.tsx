export const APP_SHOP = {
  name: "Kritika Computer V Stationary",
  phoneNumber: "+91 9413324115",
  address: "Shop no : 39-40, Opposite Garg Kiryana Store, Vridh Ashram Road",
  city: "Sri Ganganagar",
  state: "Rajashtan",
  email: "kritika.kc57@gmail.com",
  shopNo: "39,40",

  termsAndConditions: [
    {
      id: 1,
      text: "Service charges once paid are non-refundable.",
    },
    {
      id: 2,
      text: "Goods once sold will not be taken back.",
    },
    {
      id: 3,
      text: "Intrest @ 18% p.a. will be charged if payment Is not made with in the stipulated time.",
    },
    {
      id: 4,
      text: "Subject to 'Sri Ganganagar' Jurisdiction only.",
    },
  ],

  googleMapLink: "https://maps.app.goo.gl/6qXkwHQXZgnUpRDp9",

  bank: {
    accountName: "Kritika Computer V Stationary",
    accountNumber: 1628102100001929,
    ifscCode: "PUNB0162810",
  },

  GST: "08GGMPS5773Q1Z0",
};
